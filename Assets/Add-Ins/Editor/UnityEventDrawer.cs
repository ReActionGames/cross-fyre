...
public class UnityEventDrawer : PropertyDrawer
{
    ...
    private static MethodInfo findMethod = typeof(UnityEventBase).GetMethod("FindMethod",
        BindingFlags.NonPublic | BindingFlags.Instance, null, CallingConventions.Standard,
        new Type[] {typeof(string), typeof(object), typeof(PersistentListenerMode), typeof(Type)}, null);

    private static MethodInfo temp = typeof(GUIContent).GetMethod("Temp", BindingFlags.NonPublic | BindingFlags.Static,
        null, CallingConventions.Standard, new Type[] {typeof(string)}, null);
    ...
    private static string GetEventParams(UnityEventBase evt)
    {
        var method = (MethodInfo) findMethod.Invoke(evt,
            new object[] {"Invoke", evt, PersistentListenerMode.EventDefined, null});
    ...
    private void DrawEventListener(Rect rect, int index, bool isactive, bool isfocused)
    {
                 ...
                 !propertyRelative6.serializedObject.isEditingMultipleObjects)
        {
            ...
            var method = GetMethod(m_DummyEvent, propertyRelative5.stringValue, propertyRelative4.objectReferenceValue,
    ...
    private static UnityEventBase GetDummyEvent(SerializedProperty prop)
    {
        ...
        var typeProp = prop.FindPropertyRelative("m_TypeName");
        if (typeProp == null)
            return new UnityEvent();

        var type = Type.GetType(typeProp.stringValue, false);
        if (type == null)
            return new UnityEvent();
    ...
    public static bool IsPersistantListenerValid(UnityEventBase dummyEvent, string methodName,
        UnityEngine.Object uObject, PersistentListenerMode modeEnum, Type argumentType)
    {
    ...
    private static void AddMethodsToMenu(GenericMenu menu, SerializedProperty listener, List<ValidMethodMap> methods,
        string targetName)
    {
        ...
        public void Assign()
        {
            ...
            if (m_Mode == PersistentListenerMode.Object)
            {
                ...
                propertyRelative5.stringValue =
                    parameters.Length != 1 || !typeof(UnityEngine.Object).IsAssignableFrom(parameters[0].ParameterType)
